...
class Lane:
  ...
  def find_angle_vesc(self):
    ...
    if center_offset > 0:
      theta = np.arctan(center_lane/center_offset)
      return theta
    if center_offset < 0:
      theta = 1 - (-1*(abs(np.arctan(center_lane/center_offset))))
      return theta
...
def main():
  ...
  while cap.isOpened():
    ...
    if success:
      ...
      vesc_angle = rad_angle/pi
      print(vesc_angle)
      print(str(lane_obj.center_offset)[:7]+' cm')
