...
# poopybutt = VESC(serial_port='COM3')
...
class Lane:
  ...
  def __init__(self, orig_frame):
    ...
    self.roi_points = np.float32([
      (int(0.2*width),int(0.05*height)), # Top-left corner
      (0, height-1), # Bottom-left corner			
      (int(0.958*width),height-1), # Bottom-right corner
      (int(0.8*width),int(0.05*height)) # Top-right corner
    ])
  ...
  def calculate_car_position(self, print_to_terminal=False):
    ...
    return center_offset, center_lane
  ...
  def calculate_curvature(self, print_to_terminal=False):
    ...
    left_fit_cr = np.polyfit(self.lefty * self.YM_PER_PIX, self.leftx * (
      self.XM_PER_PIX), 2)
    right_fit_cr2 = np.polyfit(self.righty * self.YM_PER_PIX, self.rightx * (
      self.XM_PER_PIX), 2)
  ...
  def display_curvature_offset(self, frame=None, plot=False):
    ...
    cv2.putText(image_copy,'Lane_' + str('Detection.Boombastic'), (int((
      5/600)*self.width), int((
      20/338)*self.height)), cv2.FONT_HERSHEY_SIMPLEX, (float((
      0.5/600)*self.width)),(
      255,255,255),2,cv2.LINE_AA)
    cv2.putText(image_copy,'Center Offset: '+ str(
      ...
      0.5/600)*self.width)),(
      255,255,255),2,cv2.LINE_AA)
    cv2.putText(image_copy,'^This Needs Lots of Work^', (int((
      5/600)*self.width), int((
  ...
  def find_angle_vesc(self):
    ...
    center_lane = (bottom_right - bottom_left)/2 + bottom_left 
    center_offset = (np.abs(car_location) - np.abs(center_lane)) * self.XM_PER_PIX * 100
    if car_location > center_lane:
      theta = np.arctan(180/(car_location - center_lane))
      return theta
    if car_location < center_lane:
      theta =  (np.arctan(180/(center_lane - car_location)))
      return theta
  ...
  def get_line_markings(self, frame=None):
    ...
    #                           np.uint8))
    #Blue mask
    lower_threshold = np.uint8([94, 80, 2])
    upper_threshold = np.uint8([126, 255, 120])
    blue_mask = cv2.inRange(hls, lower_threshold, upper_threshold)
    #Yellow color mask
    lower_threshold = np.uint8([0, 150, 0])
    upper_threshold = np.uint8([255, 255, 80])
    white_mask = cv2.inRange(hls, lower_threshold, upper_threshold)
    ...
    # If you show rs_binary visually, you'll see that it is not that different 
    # from this return value. The edges of lane lines are thin lines of pixels.
    self.lane_line_markings = cv2.bitwise_or(mask, sxbinary.astype(np.uint8)) 
...
def poopyButtTimeout(seconds, vesc, angle, throttle):
  vesc.run(angle/2, throttle/2)
  time.sleep(seconds)
  vesc.run(0.5, 0.1)
...
def main():
  ...
  while cap.isOpened():
    ...
    if success:
      ...
      # rad_angle = lane_obj.find_angle_vesc()
      # print('radian angle: ', rad_angle)
      # degrees_angle = (rad_angle*180)/(np.pi)
      # print('degree angle: ', degrees_angle)
      # if lane_obj.find_center_offset() < 0:
      #   print("offset ", lane_obj.find_center_offset())
      #   vesc_angle = 0.5 + (0.5 - degrees_angle/180)
      # if lane_obj.find_center_offset() > 0:
      #   print("offset ", lane_obj.find_center_offset())
      #   vesc_angle = degrees_angle/180
      # print(vesc_angle, " vesc angle")
      # VESC.run(poopybutt, vesc_angle, 0.2)
